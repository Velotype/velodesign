...
const propTables: Record<string, PropDoc[]> = {
    ...
    DataTable: [
        ...
        {name: "columnToggleChildren", type: "RenderableElements", description: "Content of that button (default: DataTableThemeOptions.columnsSymbol, the ▥ glyph - the library never defaults visible text to a language). Pass your own word for it, or override the theme option once for your whole app."},
        {name: "columnToggleLabel", type: "string", description: "Accessible name for the column-visibility button. No default. Set it whenever the button's content is a symbol rather than words, or it has no name at all."},
        {name: "pageSizeLabel", type: "RenderableElements", description: "Caption beside the page-size control. No default - nothing iconographic means \"rows per page\", so the library renders no caption rather than assuming English."},
        {name: "emptyMessage", type: "RenderableElements", description: "Shown in place of the rows when nothing matches (default: DataTableThemeOptions.emptySymbol, the ∅ glyph)."},
    ...
    AsyncDataTable: [
        {name: "columns", type: "AsyncDataTableColumnType<RowType>[]", description: "Column definitions. Note sortable: true rather than sortValue/filterValue - the server does both, and sorting a single page in the browser would reorder only the rows that happened to come back (required)."},
        {name: "load", type: "(query) => Promise<{rows, total?, truncated?}>", description: "Fetches one page. Called on mount and on every search, sort, page and page-size change. Give total for numbered pagination; omit it for prev/next. Set truncated when the backend capped the result, and the table says so instead of looking complete (required)."},
        {name: "pageSize", type: "number", description: "Rows per page (default: 25)."},
        {name: "pageSizeOptions", type: "number[]", description: "Options offered by the page-size control, when shown (default: [10, 25, 50, 100])."},
        {name: "showPageSizeControl", type: "boolean", description: "Shows a control letting the user change how many rows are requested per page (default: false)."},
        {name: "searchable", type: "boolean", description: "Shows the search box (default: true)."},
        {name: "searchPlaceholder", type: "string", description: "Placeholder for the search input. No default text."},
        {name: "debounceMs", type: "number", description: "How long to wait after the last keystroke before calling load (default: 200)."},
        {name: "emptyMessage", type: "RenderableElements", description: "Shown when the loader returns nothing and the search box is empty - \"there is nothing here\" (default: DataTableThemeOptions.emptySymbol, the ∅ glyph)."},
        {name: "columnToggleChildren", type: "RenderableElements", description: "Content of the column-visibility button (default: DataTableThemeOptions.columnsSymbol, the ▥ glyph)."},
        {name: "columnToggleLabel", type: "string", description: "Accessible name for that button. No default - set it whenever the button's content is a symbol."},
        {name: "pageSizeLabel", type: "RenderableElements", description: "Caption beside the page-size control. No default, for the same reason as DataTable's."},
        {name: "noMatchMessage", type: "RenderableElements", description: "Shown when a non-empty search returns nothing. Kept separate from emptyMessage because an empty collection and a search that matched nothing are different facts."},
        ...
        {name: "loadingLabel", type: "string", description: "Accessible label for the loading spinner. No default - the library doesn't assume a language."},
        ...
        {name: "zebra", type: "boolean", description: "Alternate row background colors for readability (default: false)."},
        {name: "highlightOnHover", type: "boolean", description: "Highlight a row's background on hover (default: true)."},
        {name: "rowHref", type: "(row) => string | undefined", description: "Wraps a row's first visible column in a real, full-width link. Identical to DataTable's."},
        {name: "onRowSelect", type: "(row) => void", description: "Makes a row a JS-driven full-width \"select\" action instead of a link. Identical to DataTable's."},
    ],
    LineChart: [
        {name: "data", type: "ChartPointType[]", description: "One entry per category: {label, values: {seriesKey: number}} (required)."},
        {name: "series", type: "ChartSeriesType[]", description: "The series to plot: {key, label?, color?}. Their order picks their palette slots (required)."},
        {name: "area", type: "boolean", description: "Fills under each line (default: false). AreaChart is this component with it set."},
        {name: "stacked", type: "boolean", description: "Stacks series on top of each other rather than overlaying them (default: false)."},
        {name: "showDots", type: "boolean", description: "A dot at each point (default: true below 40 points)."},
        {name: "beginAtZero", type: "boolean", description: "Starts the value axis at zero rather than the data's own minimum (default: true)."},
        {name: "height", type: "number", description: "Drawing height in px, excluding the legend (default: 220). Width always fills the container."},
        {name: "formatValue", type: "(n: number) => string", description: "Formats axis ticks and tooltip values (default: String)."},
        {name: "ariaLabel", type: "string", description: "Accessible name, used as the caption of the visually-hidden data table a screen reader reads in place of the drawing. No default. The chart never sets an SVG <title>, so it triggers no native browser tooltip."},
        {name: "emptyMessage", type: "string", description: "Shown when there is nothing to draw (default: ChartThemeOptions.emptySymbol, the U+2205 glyph)."},
        {name: "hideLegend", type: "boolean", description: "Hides the legend even with more than one series (default: false; a single series never gets one)."},
    ],
    AreaChart: [
        {name: "(all of LineChart's)", type: "LineChartAttrsType", description: "AreaChart is LineChart with area set, so every attr above applies - data, series, stacked, showDots, beginAtZero, height, formatValue, ariaLabel, emptyMessage, hideLegend. The only one it does not take is area itself, which is what this component sets."},
        {name: "stacked", type: "boolean", description: "Worth calling out here: stacking is what makes an area chart say \"these parts add up to this total\" rather than \"these two lines happen to overlap\"."},
    ],
    BarChart: [
        {name: "data", type: "ChartPointType[]", description: "One entry per category (required)."},
        {name: "series", type: "ChartSeriesType[]", description: "The series to plot (required)."},
        {name: "horizontal", type: "boolean", description: "Bars run left-to-right (default: false). Worth reaching for when category labels are words rather than dates - each label gets a full line instead of being rotated."},
        {name: "stacked", type: "boolean", description: "Stacks each category's series into one bar (default: false)."},
        {name: "height", type: "number", description: "Drawing height in px (default: 220)."},
    ],
    PieChart: [
        {name: "data", type: "PieSliceType[]", description: "The slices: {label, value, color?}, drawn from 12 o'clock in the order given (required)."},
        {name: "donut", type: "number", description: "Hole size as a fraction of the radius, 0-0.9 (default: 0). 0.6 is the usual donut."},
        ...
        {name: "showPercent", type: "boolean", description: "Shows each slice's share in the tooltip (default: true)."},
    ],
    Gauge: [
        {name: "value", type: "number", description: "The value to show (required)."},
        {name: "min / max", type: "number", description: "The range (default: 0 and 100)."},
        {name: "sweep", type: "number", description: "How much of a circle the track spans, in degrees (default: 240). 180 is a half-circle for a wide, short tile."},
        ...
        {name: "label / subLabel", type: "string", description: "Centre text (label defaults to the formatted value)."},
        {name: "showRange", type: "boolean", description: "Draws min and max at the ends of the track (default: true)."},
    ],
    Sparkline: [
        {name: "values", type: "number[]", description: "The values, oldest first (required)."},
        {name: "variant", type: '"line" | "area" | "bar"', description: "How to draw them (default: line)."},
        {name: "width / height", type: "number", description: "Fixed size in px (default: 120 x 28). A sparkline does not resize with its container - it is a glyph."},
        {name: "color", type: "string", description: "CSS colour for the stroke or fill (default: var(--primary))."},
        {name: "showLast", type: "boolean", description: "A dot on the most recent value (default: true for line/area)."},
        {name: "ariaLabel", type: "string", description: "Accessible name. No default - unset, the sparkline is aria-hidden, which is right when the number it illustrates is already in the text beside it."},
    ],
    Carousel: [
...
export const groupOrder = ["Form", "Navigation", "Feedback", "Overlays", "Data Display", "Charts", "Data Entry", "Utility"]
