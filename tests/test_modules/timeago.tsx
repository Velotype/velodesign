...
class TimeAgoGallery extends Component<EmptyAttrs> {
    override render() {
        const now = new Date()
        const delta = (deltaSeconds: number) => {
            return new Date((now.getTime() + (deltaSeconds*1000))).toISOString()
        }
        return <div id="timeago-page">
            <div>{delta(4000000)}: <span id="plus-4000000"><TimeAgo timestamp={delta(4000000)}/></span></div>
            <div>{delta(400000)}: <span id="plus-400000"><TimeAgo timestamp={delta(400000)}/></span></div>
            <div>{delta(160)}: <span id="plus-160"><TimeAgo timestamp={delta(160)}/></span></div>
            <div>{delta(62)}: <span id="plus-62"><TimeAgo timestamp={delta(62)}/></span></div>
